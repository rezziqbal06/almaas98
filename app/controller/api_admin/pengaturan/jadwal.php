...
class Jadwal extends JI_Controller
{
	...
	public function __construct()
	{
		...
		$this->load("api_admin/c_jadwal_model", 'cjm');
		$this->lib("seme_upload", 'se');
		$this->hari = [1 => "senin", 2 => "selasa", 3 => "rabu", 4 => "kamis", 5 => "jumat", 6 => "sabtu", 7 => "minggu"];
	}
	...
	public function index()
	{
		...
		$datatable = $this->cjm->datatable()->initialize();
		$dcount = $this->cjm->count($datatable->keyword(), $is_active);
		$ddata = $this->cjm->data(
			$datatable->page(),
			$datatable->pagesize(),
	...
	public function baru()
	{
		...
		$data = new \stdClass();
		if (!$this->cjm->validates()) {
			$this->status = 444;
			$this->message = API_ADMIN_ERROR_CODES[$this->status];
			$validation_message = $this->cjm->validation_message();
		...
		$this->cjm->columns['cdate']->value = 'NOW()';
		$this->cjm->columns['hari']->value = $this->hari[$this->cjm->columns['day']->value];

		$res = $this->cjm->save();
		if ($res) {
			$resUpload = $this->se->upload_file('gambar', 'kategori', $res);
			if ($resUpload->status == 200) {
				$this->cjm->update($res, ['gambar' => $resUpload->file]);
			}
			$this->status = 200;
			$this->message = API_ADMIN_ERROR_CODES[$this->status];
		} else {
	...
	public function edit($id = "")
	{
		...
		$du = [];
		$du['a_pengguna_id'] = $_POST['a_pengguna_id'];
		$du['a_kategori_id'] = $_POST['a_kategori_id'];
		$du['day'] = $_POST['day'];
	...
	public function hapus($id)
	{
		...
		$res = $this->cjm->update($id, array('is_deleted' => 1));
		if ($res) {
			$this->status = 200;
			$this->message = API_ADMIN_ERROR_CODES[$this->status];
		} else {
	...
	{
		$this->load("api_admin/b_user_model", 'cjm');
		$d = $this->__init();
		$keyword = $this->input->request('q');
		$ddata = $this->cjm->select2($keyword);
		$datares = array();
		$i = 0;
