			...
			"fnServerData"	: function (sSource, aoData, fnCallback, oSettings) {
				oSettings.jqXHR = $.ajax({
					...
					$('#drTable > tbody').on('click', 'tr', function (e) {
						...
						ieid = id;
						$.get('<?=base_url("api_admin/pengaturan/jadwal/detail/")?>'+ieid).done(function(dt){
							if(dt.data){
						...
						$("#adetail").attr("href","<?=base_url_admin("pengaturan/jadwal/detail/")?>"+ieid);
						$("#asiteplan").attr("href","<?=base_url_admin("pengaturan/jadwal/siteplan/")?>"+ieid);
...
$("#ftambah").on("submit",function(e){
	...
	var url = '<?= base_url("api_admin/pengaturan/jadwal/baru/")?>';
...
$("#fedit").on("submit",function(e){
	...
	var fd = new FormData($(this)[0]);
	var url = '<?=base_url("api_admin/pengaturan/jadwal/edit/")?>';
...
$("#bhapus").on("click",function(e){
	...
	if(ieid){
		...
		if(c){
			...
			$('.btn-submit').prop('disabled',true);
			$('.icon-submit').addClass('fa-circle-o-notch fa-spin');
			var url = '<?=base_url('api_admin/pengaturan/jadwal/hapus/')?>'+ieid;
...
$("#fl_a_company_id_parent").select2({
	ajax: {
		method: 'post',
		url: '<?=base_url("api_admin/pengaturan/jadwal/get_parent/")?>',
		dataType: 'json',
    delay: 250,
