...
if(jQuery('#drTable').length>0){
	...
	.on('preXhr.dt', function ( e, settings, data ){
		$().btnSubmit();
			...
			"bProcessing"		: true,
			"bServerSide"		: true,
			"sAjaxSource"		: "<?=base_url("api_admin/pengaturan/three_d/")?>",
			...
			"fnServerData"	: function (sSource, aoData, fnCallback, oSettings) {
				oSettings.jqXHR = $.ajax({
					...
					$('#drTable > tbody').on('click', 'tr', function (e) {
						...
						ieid = id;
						$.get('<?=base_url("api_admin/pengaturan/three_d/detail/")?>'+ieid).done(function(dt){
							if(dt.data){
						...
						$("#adetail").attr("href","<?=base_url_admin("pengaturan/three_d/detail/")?>"+ieid);
						//$("#aedit").attr("href","<?=base_url_admin("pengaturan/three_d/edit/")?>"+ieid);
...
$("#ftambah").on("submit",function(e){
	...
	var url = '<?= base_url("api_admin/pengaturan/three_d/baru/")?>';
...
$("#fedit").on("submit",function(e){
	...
	var fd = new FormData($(this)[0]);
	var url = '<?=base_url("api_admin/pengaturan/three_d/edit/")?>';
...
$("#bhapus").on("click",function(e){
	...
	if(ieid){
		...
		if(c){
			...
			$('.btn-submit').prop('disabled',true);
			$('.icon-submit').addClass('fa-circle-o-notch fa-spin');
			var url = '<?=base_url('api_admin/pengaturan/three_d/hapus/')?>'+ieid;
...
$("#fl_a_company_id_parent").select2({
	ajax: {
		method: 'post',
		url: '<?=base_url("api_admin/pengaturan/three_d/get_parent/")?>',
		dataType: 'json',
    delay: 250,
...
$(document).off('click', '.lihat_3d');
$(document).on('click', '.lihat_3d', function(e){
	e.preventDefault();
	var id = $(this).attr('data-id');
	var gambar = $("#"+id+"gambar").val();
	if(!gambar){
		gritter("Script gambar belum dimasukan", "warning");
		return false;
	}
	$("#detail_3d").html(gambar);
	$("#modal_3d").modal('show');
});